"""
    Adapt the sliding_window_inference monai implementation
    for multi-decoder model.
"""
...
def sliding_window_inference(
    ...
    **kwargs: Any,
    ...
    for slice_g in range(0, total_slices, sw_batch_size):
        ...
        seg_prob = predictor(window_data, *args, **kwargs).to(device)

        if isinstance(seg_prob, list):
            background = [sum([seg_prob[i].narrow(1, 0, 1) for i in range(len(seg_prob))])]
            foreground = [seg_prob[i].narrow(1, 1, 1) for i in range(len(seg_prob))]

            seg_prob = torch.cat(background + foreground, dim=1)
